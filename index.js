...
// while(true) {
    // let input = prompt('>').trim().replace('>', '');
    // if(input.startsWith("find ")) {
        // let fileToSearch = input.replace("find ", "");
        /* broadcast data to nodes and wait for 10 seconds to respnose */
        // console.log(knownNodes);
        // for(let node of knownNodes) {
            // console.log(`Sending ${fileToSearch} to ${node.ip}:${node.port}`)
            // udpClient.send(Buffer.from(`find ${fileToSearch}`), node.port, node.ip, err => {
                // if(err) { 
                    // console.log(`Error sending known nodes to ${node.ip}:${node.port} via UDP.`)
                // }
            // })
    ...
    // } else if (input.startsWith("get ")) {
        // let fileToGet = input.replace("get ", "")
...
// const server = net.createServer(onTcpClientConnected);
// server.listen(port, host, () => {
//     console.log(`Started listening on ${host}:${port} via TCP...`);
//     activeTcpClients = [];
// })

// function onTcpClientConnected(socket) {
//     let clientString = `${socket.remoteAddress}:${socket.remotePort}`;
//     console.log(`TCP client ${clientString} connected.`);
//     if(activeTcpClients.findIndex(tmpActiveClient => `${tmpActiveClient.remoteAddress}:${tmpActiveClient.remotePort}` == clientString) == -1) {
//         //tcp client is new good.
//         activeTcpClients.push(socket)

//         socket.on('data', data => {
//             tcpServerMessageController(data);
//         })
    
//         socket.on('close', () => {
//             console.log(`${clientString} Terminated the connection`);
//             activeTcpClients = activeTcpClients.filter(tempActiveClient => `${tmpActiveClient.remoteAddress}:${tmpActiveClient.remotePort}` != clientString);
//         })
    
//         socket.on('error', error => {
//             console.error(`${clientString} connection Error ${error}`);
//         })
//     } else {
//         //tcp client already here block.
//         socket.close();
//     }
// }